...
if(update_payments){
  raw_files <-
    ...
    {tibble::tibble(
      ...
      outfile = file.path("fsa-payment-files/raw", 
                          basename(.))
    )}
  ...
  out %>%
    dplyr::group_by(`Accounting Program Year`,
                    `State FSA Name`) %>%
    arrow::write_dataset(path = "fsa-payment-files/parquet/BY_YEAR",
                         format = "parquet",
  ...
  out %>%
    dplyr::group_by(`State FSA Name`,
                    `County FSA Name`) %>%
    arrow::write_dataset(path = "fsa-payment-files/parquet/BY_COUNTY",
                         format = "parquet",
                         existing_data_behavior = "delete_matching",
                         version = "latest",
                         max_partitions = 4000L,
...
library(magrittr)
fsa_payment_files_by_county <-
  arrow::s3_bucket("climate-smart-usda/fsa-payment-files/parquet/BY_COUNTY",
                   access_key = keyring::key_get("aws_access_key_id"),
                   secret_key = keyring::key_get("aws_secret_access_key")) %>%
  arrow::open_dataset()

fsa_payment_files_by_county %>%
  dplyr::filter(`State FSA Name` %in% c(
    "Montana",
    "North Dakota",
    "South Dakota",
    "Nebraska",
  ...
  dplyr::arrange(`Formatted Payee Name`, `Accounting Program Description`, `Accounting Program Year`) %>%
  dplyr::collect() %>%
  readr::write_csv("NGP_payments.csv")

fsa_payment_files_by_year <-
  arrow::s3_bucket("climate-smart-usda/fsa-payment-files/parquet/BY_YEAR",
                 access_key = keyring::key_get("aws_access_key_id"),
                 secret_key = keyring::key_get("aws_secret_access_key"))

arrow::open_dataset(fsa_payment_files_by_year) %>%
  dplyr::filter(`Accounting Program Year` %in% c(2012, 2017, 2022)) %>%
  dplyr::collect() %>%
